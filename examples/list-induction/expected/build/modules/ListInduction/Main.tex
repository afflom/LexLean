...
For every \(\operatorname{list}(T)\) \(xs\) and \(\operatorname{list}(T)\) \(ys\), \(\texttt{example.list::equals}(\texttt{example.list::length}(xs ++ ys), \texttt{example.list::sum}(\texttt{example.list::length}(xs), \texttt{example.list::length}(ys)))\).
\end{theorem}
\begin{proof}
