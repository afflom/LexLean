...
\[\mathrm{Parameters}: \forall T \in \text{type}\]
\begin{lemma}
...
Case \(\operatorname{nil}\):
The goal follows by reflexivity.
Case \(\operatorname{cons}\) with \(x\), \(rest\), \(ih\):
Rewrite the goal using \(\rightarrow \texttt{Main::cons-append}(T, x, rest, \operatorname{nil})\), \(\rightarrow ih\).
\end{proof}
\begin{theorem}
