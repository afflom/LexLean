...
The goal follows by reflexivity.
Branch \(2\):
The goal follows from \(\operatorname{zero_add}(n)\).
\end{proof}
...
\end{corollary}
\begin{proof}
The goal follows from \(\operatorname{eqsymm}(\operatorname{add_comm}(a, b))\).
\end{proof}
...
\end{theorem}
\begin{proof}
Rewrite the goal using \(\leftarrow \operatorname{add_assoc}(a, b, c)\), \(\rightarrow \operatorname{add_comm}(a, b)\), \(\rightarrow \operatorname{add_assoc}(b, a, c)\).
\end{proof}
\end{document}
