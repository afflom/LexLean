...
For every natural number \(n\), \(\texttt{peano.arith::positive}\) holds exactly when \(0 < n\).
\end{definition}
\begin{theorem}
\label{ll:order:successor-positive}
For every natural number \(n\), \(\operatorname{positive}(\texttt{peano.arith::successor}(n))\).
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
Rewrite the goal using \(\rightarrow \operatorname{add_zero}(n)\).
...
For every natural number \(n\), \(n = 0\) or \(\exists m, n = \operatorname{succ}(m)\).
\end{theorem}
\begin{proof}
