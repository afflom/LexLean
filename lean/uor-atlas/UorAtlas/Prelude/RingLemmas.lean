...
the Lean targets". This module discharges the eight that are purely ring- or
field-theoretic and consume no Atlas data: `RH1`, `TI1`, `BC1`, `SD1`, `QL1`,
`DV1`, `LI1`, `FR1`.

Of the remaining four, `FI1` is already proved in
`UorAtlas.Prelude.NumInstances` and is imported here to discharge the
injectivity hypothesis of `TI1`; `IG1`, `RC1` and `RP1` are elsewhere, because
each needs data this module does not have -- a generated subgroup, a finite
group acting on `Sym^2(RR^O)`, and the restricted product of `D38`.
